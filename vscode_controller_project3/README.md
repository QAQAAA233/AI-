...
### 後端改進
- **📦 模塊化架構設計**
  - `core.py`：資料模型、配置管理、Gemini AI 與 JSON 解析
  - `automation.py`：VS Code 控制、螢幕擷取與程式執行管理
  - `process_manager.py`：主流程協調與偵錯輸出整合
  - `app.py`：Flask 伺服器與前端溝通層
...
│
├── app.py                  # Flask 主程式與路由
├── core.py                 # 核心資料模型、配置與 AI 服務
├── automation.py           # VS Code、自動化與螢幕擷取
├── process_manager.py      # 自動化流程協調器
...
- **`core.py`**：集中管理資料模型、Gemini AI 呼叫、JSON 解析、全域日誌設定與檔案路徑常數，確保所有高頻共用變數在單一檔案維護。
- **`automation.py`**：封裝 VS Code 視窗控制、螢幕擷取與程式啟動流程，並整合詳細的錯誤日誌與視窗偵錯資訊。
- **`process_manager.py`**：協調 AI 產出解析、檔案儲存、套件安裝與程式執行，統一輸出格式與除錯訊息。
- **`app.py`**：專注於 Flask 路由與桌面 Webview 溝通，呼叫前述模組完成任務，維持前後端資料流一致。
